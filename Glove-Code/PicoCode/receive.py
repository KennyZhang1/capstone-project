...
past_time = utime.time()
distances = []
while True:
    current_time = utime.time()
    t_delta = current_time-past_time
    
    if t_delta % 5 == 0:
        print("switching: ", scanning_phase)
        scanning_phase = not(scanning_phase)
...
#         if len(distances) > 0:
#             advertisedData = bytes(distances[0]) # the integer to send as advertised data
#         else:
        advertisedData = b'juicy feet'
